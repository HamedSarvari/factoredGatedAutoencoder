...
from utils_gmm.rng import np_rng, py_rng


class ToyDataset():
    def __init__(self, samples, target):
        self.data = {"samples":samples, "label":target}
...
def tensor_shuffle(data1, data2):
    print(data1)
    idxs = np_rng.permutation(np.arange(data1.shape[0]))
    return data1[idxs], data2[idxs]
